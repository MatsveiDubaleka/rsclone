...
const Header = () => {
  ...
  const onChange = (e: {
    ...
    axios
      .get(
        `https://kinopoiskapiunofficial.tech/api/v2.1/films/search-by-keyword?keyword=${e.target.value}&page=1`,
        {
          headers: {
            'X-API-KEY': token,
          },
      ...
      .then(({ data }) =>
        !data.errors ? setResults(data.films) : setResults([])
      );
  };
      ...
      <header className='header'>
        ...
        <div className='header-nav'>
          ...
          <div className='user-auth'>
            <Link
              to={
                getUsernameFromLocalStorage() !== 'admin'
                  ? `/my-account`
                  : `/admin-account`
              }
              className={`avatar-link ${isLogIn ? '' : 'hidden'}`}
            >
