...
const MovieVideo = () => {
  ...
  useEffect((): void => {
    axios
      .get<TrailersArray[]>(
        `https://rskinopoisk.herokuapp.com/trailer/getTrailer`,
        {
          headers: {
      ...
      .then(({ data }: AxiosResponse<TrailersArray[]>): void => {
        const filteredData: TrailersArray[] = data.filter(
          (x: { nameEn: string }): boolean => x.nameEn.length > 0
        );
        ...
        if (filmName !== undefined) {
          axios
            .get(
              `https://kinopoiskapiunofficial.tech/api/v2.1/films/search-by-keyword?keyword=${filmName}&page=1`,
              {
                headers: {
                  'X-API-KEY': token,
                },
          ...
          axios
            .get(
              `https://kinopoiskapiunofficial.tech/api/v2.2/films/premieres?year=${currentYear}&month=${currentMonth}`,
              {
                headers: {
                  'X-API-KEY': token,
                },
  ...
  movieTrailer(nameEn)
    .then((response: string): void => {
      response === null
        ? setVideoURL('https://youtu.be/5bqpcIX2VDQ')
