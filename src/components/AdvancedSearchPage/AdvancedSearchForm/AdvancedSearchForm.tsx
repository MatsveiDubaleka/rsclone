		...
		<div className="advanced-search-form">
			...
			<form onChange={handleSubmit(onSubmit)} className="advanced-search-form__form" id="advanced-search-form__form" >
				<div className="advanced-search-form__row">
					...
					<input id="search-form-film-year" className="advanced-search-form__row_film-year" type="text" autoComplete="off" {...register("year", { min: 1900, max: 2022 })} />
					<label htmlFor="search-form-film-minYear" className="advanced-search-form__row_label-minYear">...или интервал:</label>
					<input id="search-form-film-minYear" className="advanced-search-form__row_film-minYear" type="text" autoComplete="off" {...register("yearFrom", { min: 1900, max: 2022 })}/>
					<input id="search-form-film-maxYear" className="advanced-search-form__row_film-maxYear" type="text" autoComplete="off" {...register("yearTo", { min: 1900, max: 2022 })} />
				</div>
				<div className="advanced-search-form__row">
					...
					<select {...register("countries")} id="search-form-film-country" className="advanced-search-form__row_film-country">
						{countries.map((country: {id: number, country: string}) : JSX.Element => {
							return <option id={`${country.id}`} value={country.country} key={`country-${country.id}`}>{country.country}</option>
						})}
					</select>
					<label htmlFor="search-form-film-rating">Рейтинг</label>
					<input id="search-form-film-rating" className="advanced-search-form__row_film-rating" type="text" autoComplete="off" {...register("rating")}/>
					<label htmlFor="search-form-film-minRating" className="advanced-search-form__row_label-minRating">...или интервал:</label>
					<input id="search-form-film-minRating" className="advanced-search-form__row_film-minRating" type="text" autoComplete="off" {...register("ratingFrom")} />
					<input id="search-form-film-maxRating" className="advanced-search-form__row_film-maxRating" type="text" autoComplete="off" {...register("ratingTo")} />
				</div>
				<div className="advanced-search-form__row">
					...
					<select {...register("genres")} id="search-form-film-genre" className="advanced-search-form__row_film-genre">
						{genres.map((genre: {id: number, genre: string}) : JSX.Element => {
							return <option className="advanced-search-form__row_film-genre_option" id={`${genre.id}`} value={genre.genre} key={`genre-${genre.id}`}>{genre.genre}</option>
						})}
					</select>
				</div>
				<Link to={`/searchPage?keyword=${searchArray?.keyword}
							...
							&counties=${searchArray?.countries}&genres=${searchArray?.genres}`}>
					<button onClick={()=>{handleSubmit(onSubmit)}} type="submit" className="advanced-search-form__button-search">Искать</button>
				</Link>
			</form>
		</div>
	)
