...
const MovieCardFlat : FC<IDropdownMovie> = ({num, nameRu, nameEn, posterUrlPreview, year, rating, ratingKinopoisk=NaN, filmId, kinopoiskId,  filmLength, genres, countries}) => {
		...
		<li className="movie-card-flat" id={`${filmId ? filmId : kinopoiskId}`}>
			...
			<div className="movie-card-flat__poster">
				<div className="movie-card-flat__poster-image" style={{backgroundImage: `url(${posterUrlPreview})`}}>
					<div className={`movie-card-flat__poster-rating ${getClassForRaiting(rating ? rating : ratingKinopoisk)}`}>
						{String(rating ? rating : ratingKinopoisk) !== 'null' 
						&& String(rating ? rating : ratingKinopoisk) !== 'undefined'
						&& rating ? rating : ratingKinopoisk === 0 ? String(rating ? rating : ratingKinopoisk) : '-'}
					</div>
				</div>
			</div>
			<div className='movie-card-flat__info'>
				...
				<p className={`movie-card-flat__rating ${getClassForRaiting(rating ? rating : ratingKinopoisk)}`}>
					{String(rating ? rating : ratingKinopoisk) !== 'null' &&
					String(rating ? rating : ratingKinopoisk) !== 'undefined'
					&& rating ? rating : ratingKinopoisk === 0 ? rating ? rating : ratingKinopoisk : '-'}
				</p> 
	...
	return (
			<ul>
				{movies.map((movie: IDropdownMovie, index: number) : JSX.Element => {
					...
					rating={movie.rating ? movie.rating : movie.ratingKinopoisk} posterUrlPreview={movie.posterUrlPreview}
					filmLength={movie.filmLength} genres={movie.genres}
					countries={movie.countries} filmId={movie.filmId ? movie.filmId : movie.kinopoiskId} key={`dropdown-movie-item-${index}`} />
				})}
			</ul>
	)
}
